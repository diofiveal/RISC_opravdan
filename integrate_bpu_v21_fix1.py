...
# ----------------------------------------------------------------------
# 1. Latest BPU, plus C.J/C.JAL quadrant correction.
#    Use regex here because the predictor branch changed whitespace several
#    times while keeping the same RTL expression.
# ----------------------------------------------------------------------
print("[2/6] Installing latest BPU + IFU...")
bpu = git_show(BPU)

# C.J/C.JAL are quadrant 01.  If upstream already contains 01, leave it alone.
if re.search(r"assign\s+ch1_is_rvc_jump\b[\s\S]*?ch1_lo\[1:0\]\s*==\s*2'b10", bpu):
    bpu = regex_replace_once(
        bpu,
        r"(assign\s+ch1_is_rvc_jump\b[\s\S]*?ch1_lo\[1:0\]\s*==\s*)2'b10",
        r"\g<1>2'b01",
        "BPU C.J ch1 quadrant"
    )
elif not re.search(r"assign\s+ch1_is_rvc_jump\b[\s\S]*?ch1_lo\[1:0\]\s*==\s*2'b01", bpu):
    die("BPU C.J ch1 decoder was not recognized")

if re.search(r"assign\s+ch2_is_rvc_jump\b[^\n]*ch2_lo\[1:0\]\s*==\s*2'b10", bpu):
    bpu = regex_replace_once(
        bpu,
        r"(assign\s+ch2_is_rvc_jump\b[^\n]*ch2_lo\[1:0\]\s*==\s*)2'b10",
        r"\g<1>2'b01",
        "BPU C.J ch2 quadrant"
    )
elif not re.search(r"assign\s+ch2_is_rvc_jump\b[^\n]*ch2_lo\[1:0\]\s*==\s*2'b01", bpu):
    die("BPU C.J ch2 decoder was not recognized")
