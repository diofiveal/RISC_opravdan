...
always_ff @(posedge clk) begin
    if (bpu_steer) begin
        $display("[IFU-DBG] STEER: resp_addr=%h imem_addr=%h target=%h pred_taken=%b pred_vld=%b discard_cnt=%0d pnd=%0d q_w=%0d q_r=%0d empty=%b free=%0d straddle=%b",
            {bpu_resp_word_addr, 2'b00}, {imem_addr_ff, 2'b00}, bpu_target, bpu_predict_taken, bpu_pred_vld,
            imem_resp_discard_cnt, imem_pnd_txns_cnt, q_wptr, q_rptr, q_is_empty, q_free_h_next, instr_hi_rvi_lo_ff);
    end
    if (exu2ifu_pc_new_req_i) begin
        $display("[IFU-DBG] FLUSH_REQ: imem_addr=%h new_pc=%h q_w=%0d q_r=%0d empty=%b discard=%0d pnd=%0d steer=%b",
            {imem_addr_ff, 2'b00}, exu2ifu_pc_new_i, q_wptr, q_rptr, q_is_empty,
            imem_resp_discard_cnt, imem_pnd_txns_cnt, bpu_steer);
    end
    if (bpu_predict_taken & ~bpu_steer) begin
        $display("[IFU-DBG] NOSTEER: resp_ok=%b resp_vd=%b complete=%b flush_req=%b straddle=%b unal=%b pred_vld=%b ch1v=%b ch2v=%b pnd=%0d discard=%0d",
            imem_resp_ok, imem_resp_vd, bpu_pred_instr_complete, exu2ifu_pc_new_req_i,
            instr_hi_rvi_lo_ff, new_pc_unaligned_ff, bpu_pred_vld,
            bpu_ch1_vld, bpu_ch2_vld, imem_pnd_txns_cnt, imem_resp_discard_cnt);
    end
end
`endif // SCR1_BPU_EN
